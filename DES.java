...
public class DES {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("请输入明文/密文：");
        String text = in.next();
        System.out.println("请输入密钥：");
        String key = in.next();
        if (key== null || key.length() < 8) {
            throw new RuntimeException("加密失败，密钥不能小于8位");
        }
        if(key.length()>8){
            StringBuffer stringBuffer=new StringBuffer(key);
            stringBuffer.delete(8,key.length());
            key=stringBuffer.toString();
        }
        System.out.println("请选择加密（输入0）解密（输入1）:");
        int num=in.nextInt();
        DES des = new DES();
        if(num==0) {
            String ciphertext = des.encrypt(text, key, "encrypt");
            System.out.println("加密为：" + ciphertext);
        }else if(num==1) {
            String ciphertext = des.encrypt(text, key, "decrypt");
            System.out.println("解密为：" + ciphertext);
        }
    }
    ...
    public String encrypt(String plaintext, String key, String type) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            R.setLength(0);
            for (int j = 0; j < 8; ++j) {
                String mSNumber = mRTemp.substring(j * 6, (j + 1) * 6);//截取每一行的数字
